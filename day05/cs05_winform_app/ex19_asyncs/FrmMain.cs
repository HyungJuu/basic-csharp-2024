...
namespace ex19_asyncs
{
    public partial class FrmMain : Form
    {
        ...
        long CopySync(string srcPath,  string destPath)
        {
            ...
            {   // 원래 존재하는 파일을 열기 때문에 FileMode.Open
                using (FileStream toStream = new FileStream(destPath, FileMode.Create))
                {   // 존재하지 않는 파일을 만들기 때문에 FileMode.Create
                    // 1Mbyte 버퍼 생성
                    byte[] buffer = new byte[1024 * 1024]; // 1024(byte) = 1Kbyte, 1024 * 1024 = 1Mbyte -> 테스트시 10으로 변경
