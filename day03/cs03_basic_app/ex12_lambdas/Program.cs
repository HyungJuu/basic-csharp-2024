...
{
    delegate int Calculate(int a, int b);   // 대리자의 정수값 두개를 받아 정수값을 리턴해주는 함수들을 대신 실행하겠다 선언
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Calculate calc;
            calc = delegate (int a, int b) // 대리자로 만든 무명 메소드,
            {
                return a + b;
            };
            ...
            // 람다식 : 훨씬 짧게 코딩 가능
            // calc와 동일한 기능,return문을 생략. 단, {}(중괄호)를 쓰면 return문이 필요함
            Calculate calc2 = (a, b) => a + b; // (int a, int b) => { return  a + b; };

            Console.WriteLine($"계산 결과 = {calc2(11, 4)}");

            // 문장형식의 람다식
            Calculate calc3 = (a, b) =>
            {
                Console.WriteLine("이런 식으로 뺄셈계산이 됩니다.");
                var sum = a - b;
                return sum;
            };
