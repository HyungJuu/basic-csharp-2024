...
namespace ex18_winControlApp
{
    public partial class FrmMain : Form
    {
        ...
        public FrmMain()
        {
            ...
            GrbEditor.Text = "텍스트에디터"; // 코드비하인드 디자이셋팅
        }
        ...
        void ChangeFont()
        {
            ...
            TxtSampleText.Font = new Font((string)CboFonts.SelectedItem, 9, style);
        }
        ...
        {
            PrgDummy.Value = TrbDummy.Value; // 트랙바의 포인터를 옮기면 프로그레스바의 값도 같이 변경됨
        }
        ...
        private void BtnModal_Click(object sender, EventArgs e)
        {
            ...
            FrmModal.Text = "모달창";
            FrmModal.Width = 300; // 넓이
            FrmModal.Height = 100; // 높이
            FrmModal.BackColor = Color.LightCyan; // 배경색
            FrmModal.StartPosition = FormStartPosition.CenterParent;
            FrmModal.ShowDialog();  // 모달창 띄우기
        } // 자식창이 열려있을 때는 부모창의 작업을 할 수 없음(무조건 자식창을 먼저 닫고 부모창을 닫음)
        ...
        private void BtnModaless_Click(object sender, EventArgs e)
        {
            ...
            FrmModaless.Width = 300; // 넓이
            FrmModaless.Height = 100; // 높이
            FrmModaless.BackColor = Color.Lavender; // 배경색
            // 모달리스창을 부모창 정중앙에 위치시키기 위해서는 아래와 같이 작업
        ...
        } // 자식창이 열려있는 상태에서 부모창의 작업이 가능(부모창의 창닫기를 누르면 창이 모두 꺼짐)
        ...
        private void BtnLoad_Click(object sender, EventArgs e)
        {
            ...
            DlgOpenImage.Filter = "Image Files(*.bmp; *.jpg; *.png) | *.bmp; *.jpg; *.png"; // (확장자) | 확장자 한정
        ...
        {
            // OpenFileDialog 컨트롤을 디자인에서 구성하지 않고 생성하는 법
            OpenFileDialog dialog = new OpenFileDialog();
            dialog.Multiselect = false; // 여러개의 파일 선택 금지
            dialog.Filter = "Text Files(*.txt; *.cs; *.py) | *.txt; *.cs; *.py";
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                // UTF-8 -> 한글이 깨짐. EUC-KR(Window 949), UTF-8 BOM -> 한글 문제없음
        ...
        private void BtnNothread_Click(object sender, EventArgs e)
        {
            ...
            BtnThread.Enabled = BtnThread.Enabled = true;
        ...
        private void DoRealWork(BackgroundWorker worker, DoWorkEventArgs e)
        {
            ...
            for (var i = 0; i <= maxValue; i++)
            {
                if (worker.CancellationPending) // 중간에 취소할것인지 물어보는 것
                {
                    e.Cancel = true;
                    break;
                }
