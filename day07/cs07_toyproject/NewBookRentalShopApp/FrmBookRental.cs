...
namespace NewBookRentalShopApp
{
    ...
    public partial class FrmBookRental : MetroForm
    {
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            TxtMemberIdx.Text = TxtBookIdx.Text = string.Empty;
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
                {
                    ...
                    var returnDate = ""; // 반납 날짜 때문에 추가처리
                    if (DtpReturnDate.Value <= DtpRentalDate.Value) // 대출일보다 반납일이 더 뒤에 있어야 한다.
                    {
                        returnDate = "";
                    }
                    else
                    ...
                    if (result > 0)
                    {
                        ...
                        // MetroMessageBox.Show(this.Parent.Parent,  "저장성공 ^^7", "안내", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        MessageBox.Show("저장 성공", "안내", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("저장 실패", "안내", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            catch (Exception ex)
